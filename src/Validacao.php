...
class Validacao
{
    ...
    public static function tipos($real, array $esperados)
    {
        ...
        foreach ($esperados as $esperado) {
            if (mb_strpos($real, $esperado) !== false) {
                $ok = true;
                break;
            }
        }
        ...
        if (!$ok) {
            throw new \RangeException('O tipo do arquivo é inválido');
        }
    }
    ...
    public static function imagem($imagem, $largura, $altura)
    {
        if (is_string($imagem)) {
            ...
            $dimensoes = [imagesx($imagem), imagesy($imagem)];
        }
    ...
    {
        if (mb_strlen($cpf) !== 11) {
            throw new \UnexpectedValueException('#1 O CPF é inválido');
        }
    }
    ...
    private static function apenasNumeros($cpf)
    {
        if (!preg_match('/\d{11}/', $cpf)) {
            throw new \UnexpectedValueException('#2 O CPF é inválido');
        }
    }
    ...
    private static function sequenciaIgual($cpf)
    {
        for ($posicao = 1, $numero = $cpf[0]; $posicao <= 10; $posicao++) {
            if ($numero != $cpf[$posicao]) {
                break;
            }
            
            if ($posicao == 10) {
                throw new \UnexpectedValueException('#3 O CPF é inválido');
            }
        }
    }
    ...
    private static function digitosCpf($cpf)
    {
        for ($posicao = 9; $posicao <= 10; $posicao++) {
            ...
            if ($cpf[$numero] != $digito) {
                throw new \RangeException('#4 O CPF é inválido');
            }
        }
    }
    ...
    private static function quantidadeCaracteresCnpj($cnpj)
    {
        if (mb_strlen($cnpj) !== 14) {
            throw new \UnexpectedValueException('#1 O CNPJ é inválido');
        }
    }
    ...
    private static function apenasNumerosCnpj($cnpj)
    {
        if (!preg_match('/\d{14}/', $cnpj)) {
            throw new \UnexpectedValueException('#2 O CNPJ é inválido');
        }
    }
    ...
    private static function sequenciaZeros($cnpj)
    {
        if ($cnpj == '00000000000000') {
            throw new \UnexpectedValueException('#3 O CNPJ é inválido');
        }
    }
    ...
    private static function digitosCnpj($cnpj)
    {
        for ($i = 0; $i <= 1; $i++) {
            ...
            if ($cnpj[12 + $i] != $digito) {
                throw new \RangeException('#4 O CNPJ é inválido');
            }
        }
    }
}
